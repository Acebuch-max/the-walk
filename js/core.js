...
(() => {
  ...
  // --- THEWALK namespace ---------------------------------------------------------
  const THEWALK = (window.THEWALK = window.THEWALK || {});
  THEWALK.version = THEWALK.version || "core+arcilla4.v1";

  // Bus mínimo (no dependemos de frameworks)
  THEWALK.emit = (name, detail) => window.dispatchEvent(new CustomEvent(name, { detail }));
  THEWALK.on = (name, fn, opts) => window.addEventListener(name, fn, opts);
  ...
  class Arcilla4Engine {
    ...
    getDisparoConfig(){
      ...
      const THEWALKist        = 0.9 + (1 - s.entropy)*1.4;              // ~0.9..2.3

      // muestreo: determinismo alto permite más detalle (sin colapsar)
      const sample = (s.determinism > 0.62) ? 360 : 300;

      return { holdSeconds, totalSeconds, speed, THEWALKist, sample };
    }
    ...
    setBreath({ breath, phase }){
      ...
      THEWALK.emit("THEWALK:arcilla4:space", this.getSpace());
    }
    ...
    ingestPoem(text, source="dom"){
      ...
      if(t){
        ...
        THEWALK.emit("THEWALK:arcilla4:poem", { source, vector: this.poemVector });
        THEWALK.emit("THEWALK:arcilla4:space", this.getSpace());
      }
    }
  ...
  THEWALK.rng = THEWALK.rng || ((scope) => THEWALK.arcilla4.rng(scope));
  ...
  function ensureBodyClass(){
    ...
    if(!b) return;
    if(!b.classList.contains("THEWALK")) b.classList.add("THEWALK");
  }
  ...
  function setBreath(b){
    ...
    THEWALK.emit("THEWALK:breath", { breath: breathState.breath, phase });

    // Arcilla-4: pliegue inmediato de espacio
    THEWALK.arcilla4.setBreath({ breath: breathState.breath, phase });

    const dbg = document.querySelector(".THEWALK__debug");
    if(dbg) dbg.textContent = `breath ${breathState.breath.toFixed(3)} · ${phase}`;
  }

  function mountDebug(){
    if(!breathState.debug) return;
    document.body?.classList.add("THEWALK-debug");
    if(!document.querySelector(".THEWALK__debug")){
      const d = document.createElement("div");
      d.className = "THEWALK__debug";
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if(e.key === "d" || e.key === "D"){
        ...
        if(breathState.debug){
          ...
          document.body?.classList.remove("THEWALK-debug");
          document.querySelector(".THEWALK__debug")?.remove();
        }
        e.preventDefault();
      }
    });
  ...
  function ingestPoemFromDOM(){
    ...
    if(t) { THEWALK.arcilla4.ingestPoem(t, "dom:ptolomeo"); return; }

    // Umbral: parámetro t=
    const q = new URLSearchParams(location.search);
    const lit = q.get("t");
    if(lit) { THEWALK.arcilla4.ingestPoem(lit, "query:t"); return; }

    // fallback: título (como huella leve)
    const title = document.title || "";
    if(title) THEWALK.arcilla4.ingestPoem(title, "title");
  }
  ...
  THEWALK.on("THEWALK:umbral-vector", (e) => THEWALK.arcilla4.setUmbralVector(e.detail), { passive:true });
  // Partitura (estructura del acto)
  THEWALK.on("THEWALK:partitura", (e) => { THEWALK.arcilla4.partitura = e.detail || null; THEWALK.emit("THEWALK:arcilla4:space", THEWALK.arcilla4.getSpace()); }, { passive:true });

  
  // Inicialización global de Arcilla (instancia única + alias)
  THEWALK.arcilla4 = THEWALK.arcilla4 || new Arcilla4Engine();
  THEWALK.arcilla  = THEWALK.arcilla  || THEWALK.arcilla4;
